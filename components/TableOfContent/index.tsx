...
export default function TableOfContents({ headings }: Props) {
  ...
  return (
     <nav className="bg-black border-solid border-2 border-white p-4 rounded-md shadow-lg max-w-xs sticky top-20 pl-8">
      <p className="text-lg font-semibold mb-4 text-white">Table of contents</p>
      <ul className="list-disc list-outside space-y-2">
        {headings.map(({ slug, title, level }, index) => (
          <li key={index} className={`ml-${level * 2}`}>
            <a
              href={`#${slug}`}
